...
void main() {
    ...
    vec2 RealProjMat = vec2(tan(decode_float(texture(DataSampler, start + CTL_ATAN_PMAT00 * inc).xyz)), tan(decode_float(texture(DataSampler, start + CTL_ATAN_PMAT11 * inc).xyz)));

    float near = PROJNEAR;
    float far = float(decode_int(texture(DataSampler, start + CTL_FARCLIP * inc).xyz));

    ProjInv = inverse(transpose(mat4(RealProjMat.x, 0.0, 0.0, 0.0,
                                    0.0, RealProjMat.y, 0.0, 0.0,
