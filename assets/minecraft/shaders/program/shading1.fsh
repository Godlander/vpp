...
vec3 joined_bilateral_gaussian_blur_x(vec2 uv1, vec2 uv2, vec2 duv, float sigX, float sigY, float sigV, float stride) {   

    float total = 0.0;
    vec3 ret = vec3(0.0);

    float depth = decode_depth(texture(DiffuseDepthSampler, duv));
    
    for (float i = -BLF_SAMPLES * stride; i <= BLF_SAMPLES * stride; i+= stride)
    {
        float fg = gaussian(sigY, i);

        float offsetx = i * oneTexel.x;

        float sd = decode_depth(texture(DiffuseDepthSampler, duv + vec2(offsetx, 0.0)));
                    
        float fv = gaussian(sigV, abs(sd - depth));
        
        total += fg * fv;
        ret += fg * fv * vec3(texture(DiffuseSampler, uv1 + vec2(offsetx, 0.0)).rg, texture(DiffuseSampler, uv2 + vec2(offsetx, 0.0)).r);
    }
...
void main() {
    ...
    outColor = vec4(joined_bilateral_gaussian_blur_x(normCoord1, normCoord2, texCoord, st_dev, st_dev, sigV, stride), 1.0);
